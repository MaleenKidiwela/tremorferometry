#!/usr/bin/env python
"""Plot the mirror-corrected residual (no reverse) dv/v (fwd - beta*rev, deseasoned) for B926+B011, with ETS episode
...
for i, (S, la, lo) in enumerate(STATIONS):
    ...
    fwd = des(cm(f"data/daily_dvv_{TAG}_Z_2to4.csv", fams)); rev = des(cm(f"data/daily_dvv_{TAG}_MIRROR_2to4.csv", fams))
    ...
    ax.plot(resid.index, resid.values, lw=1.1, color="#1a1a2e", label="mirror-corrected residual (no reverse)")
    ...
    ax.set_title(f"{S} — mirror-corrected residual (no reverse) (β={beta:.2f}, std {resid.std():.3f}%); orange = ETS episode onsets", fontsize=11)
...
axe.set_title("Epoch stack: MIRROR-corrected residual around ETS onset (co-onset +0.043%, p=0.019, NO reverse; grey=pre, orange=co-onset window)", fontsize=11)
axe.legend(loc="upper left", fontsize=9, frameon=False)
fig.savefig("figures/mirror_corrected_dvv_ets.png", dpi=130, bbox_inches="tight")
print("-> figures/mirror_corrected_dvv_ets.png")
