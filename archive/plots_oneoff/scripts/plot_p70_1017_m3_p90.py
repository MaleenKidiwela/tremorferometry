#!/usr/bin/env python
"""Plot B011 families with P(LFE)>0.9, >=3 members, 2010-2017, no year rule."""
import matplotlib
matplotlib.use("Agg")
...
sc = ax.scatter(hi.lon, hi.lat, c=hi.p_lfe, cmap="viridis", vmin=0.9, vmax=1.0,
                s=24 + 18 * (hi.n_members - 3), edgecolor="white", lw=0.35,
                alpha=0.92, zorder=4, transform=proj)
ax.scatter([STA_LON], [STA_LAT], s=380, marker="*", color="#1a1a1a",
           edgecolor="white", lw=1.0, zorder=6, label="B011 station", transform=proj)
for m in (3, 5, 7):
    ax.scatter([], [], s=24 + 18 * (m - 3), c="#3b7a57", edgecolor="white",
               lw=0.35, label=f"{m} seed members")
...
ax.set_title(f"B011 — {len(hi)} high-confidence LFE families\n"
             "P(LFE)>0.9 · ≥3 members · 2010–2017 · no year rule",
             fontsize=12.5, pad=12)
ax.legend(loc="upper left", fontsize=9.5, framealpha=0.92)
plt.savefig("figures/b011_p70_1017_m3_p90.png", dpi=170, bbox_inches="tight")
