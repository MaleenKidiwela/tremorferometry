#!/usr/bin/env python
"""Plot the 90 solid B011 families: P(LFE)>0.8, >=4 members, 2010-2017, no year rule.
...
sm = pd.read_csv("data/b011_disc_p70_2010_2017_m4.summary.csv").set_index("family_id")
...
sc = ax.scatter(hi.lon, hi.lat, c=hi.p_lfe, cmap="viridis", vmin=0.8, vmax=1.0,
                s=30 + 22 * (hi.n_members - 4), edgecolor="white", lw=0.4,
                alpha=0.95, zorder=4, transform=proj)
ax.scatter([STA_LON], [STA_LAT], s=380, marker="*", color="#1a1a1a",
           edgecolor="white", lw=1.0, zorder=6, label="B011 station", transform=proj)

# member-size legend
for m in (4, 5, 6):
    ax.scatter([], [], s=30 + 22 * (m - 4), c="#3b7a57", edgecolor="white",
               lw=0.4, label=f"{m} seed members")
cb = plt.colorbar(sc, ax=ax, shrink=0.55, pad=0.02)
cb.set_label("family-stack P(LFE)", fontsize=10)
ax.set_title(f"B011 — 90 solid LFE families\n"
             "P(LFE)>0.8 · ≥4 members · 2010–2017 · no year rule",
             fontsize=12.5, pad=12)
ax.legend(loc="upper left", fontsize=9.5, framealpha=0.92)
plt.savefig("figures/b011_p70_1017_p80_solid90.png", dpi=170, bbox_inches="tight")
