#!/usr/bin/env python
"""T2a shallow-share slope for B018 (the NON-CIRCULAR depth discriminator; FAMILY_TRUST_TEST.md §2).
...
ACF=pd.read_csv('data/autocorr_dvv_B018.csv')           # date,dvv,cc_max  (shallow monitor)
FAM=pd.read_csv('data/daily_dvv_B018_2to4_calT.csv')    # patch,date,dvv,cc_max,n_stack
TR =pd.read_csv('data/family_trust_tier1_B018.csv'); TR=TR[TR.kind=='F']
fmax=pd.read_csv('data/family_trust_tier1_B018.csv').query("kind=='R'").coda_sigma.max()
